...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if self.path == "/api/status":
            ...
            rows = "".join(f"<tr><td>{html.escape(str(j['id']))}</td><td>{html.escape(str(j['name']))}</td><td>{html.escape(str(j['status']))}</td></tr>" for j in status.get("jobs", []))
            body = ("<html><meta charset='utf-8'><title>Milo Mission Control</title><style>body{font:16px system-ui;max-width:900px;margin:40px auto}table{width:100%;text-align:left}</style><h1>Milo Mission Control</h1><p>PID %s, IPC port %s</p><table><tr><th>ID</th><th>Job</th><th>Status</th></tr>%s</table></html>" % (status["pid"], status["port"], rows)).encode()
