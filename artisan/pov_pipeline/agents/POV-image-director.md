...
description: >
  ...
  segment. CHARACTER REGISTRY enforced. Optimized for 35-55 segment scripts.
  NO JSON.
---
...
SECONDARY OUTPUT: 05_IMAGES/IMAGE_PROMPTS_BATCH_NN.txt (split, 50 segments each).
...
sub-images. Usually fits in ONE batch file.
...
## CHARACTER REGISTRY — TOP OF EVERY BATCH FILE

Every batch file opens with VIDEO_ID + MANIFEST_HASH header, then CHARACTER
REGISTRY block, then a blank line, then the prompts.
...
- **Art Style:** Minimalist 2D vector cartoon, clean line art, solid colors,
  flat shading, corporate illustration style.
...
- **Body:** Simple geometric torso. Flat solid colors.No
  complex anatomy.
...
`Style: Minimalist 2D vector cartoon, clean line art, solid colors, flat shading, corporate illustration style. Main character: bald caucasian man, black bead eyes, no nose.`
...
1. **IMAGE_PROMPTS_BATCH_FINAL.txt:** Complete master file with ALL
   prompts. The assembler needs this.

   PROMPT SUMMARY (top of FINAL file only):
   - TOTAL SEGMENTS: [BODY/OUTRO count]
   - TOTAL PROMPTS: [individual prompts including sub-images]
   - EXPECTED FILES: [comma-separated IDs]

2. **Split Batches:** After master file is written, split into
   05_IMAGES/IMAGE_PROMPTS_BATCH_01.txt, etc. Size: 50 segments each.
   For short-form, often only one batch is needed.

First two lines of EVERY batch file: VIDEO_ID and MANIFEST_HASH verbatim.
Then blank line, CHARACTER REGISTRY, blank line, prompts.

---

## PRE-COMMIT QUALITY GATE (Short-Form Additions)
...
- [ ] Style anchor on EVERY prompt reads exactly:
      "Minimalist 2D vector cartoon, clean line art, solid colors,
...
- [ ] VIDEO_ID and MANIFEST_HASH at top.
...
[<SEGMENT_ID>] <Location>. <Character action — active verb>. Camera: [ANGLE]. Motion: [KB: CODE]. Lighting: <description>. Colors: <palette>. Mood: <vocabulary>. Focus: <focal element>. SCENE ANCHOR: <anchor text — identical across sub-images>. Character: <NAME>: <full inline DNA> (when applicable). Beat: <SETUP/DEVELOPMENT/TURN/ESCALATION/CONSEQUENCE>. Style: Minimalist 2D vector cartoon, clean line art, solid colors, flat shading, corporate illustration style. Main character: bald caucasian man, black bead eyes, no nose.
