...
def main():
    channel = (os.getenv('RANKING_UPLOAD_CHANNEL') or os.getenv('RANKING_CHANNEL') or 'rankdrop').strip()
    ...
    for row in p.db.pending_builds(limit=100):
        ...
        plan['local_path'] = path; plan['channel'] = channel
    ...
    print(f'uploaded {uploaded} pending build(s), channel={channel}, privacy=public, remaining_daily={max(0, remaining-uploaded)}')
    return 0
