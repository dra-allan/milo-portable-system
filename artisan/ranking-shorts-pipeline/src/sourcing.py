...
Metadata is fetched first. Candidate downloads are capped in duration,
resolution and bytes so vetting never fills the machine with 1080p/4K sources
that will mostly be discarded. The selected file is reused by the renderer.
"""
import re
...
def _ydl(opts: Dict):
    ...
    base={'quiet':True,'no_warnings':True,'noprogress':True,'ignoreerrors':True,
          'retries':3,'extractor_retries':3,'fragment_retries':8,
          'retry_sleep':lambda n:min(8,2**n),'socket_timeout':30,
          'concurrent_fragment_downloads':int(config.get('download_concurrency',4) or 4)}
    base.update(opts); return YoutubeDL(base)
...
def discover(topic_cfg:Dict,db,limit:Optional[int]=None)->List[Dict]:
    limit=limit or int(config.get('candidates_per_topic',40)); max_duration=float(config.get('max_source_duration',900)); min_views=int(config.get('min_source_views',500))
    queries=topic_cfg.get('queries') or []; per_query=max(5,limit//max(1,len(queries)))
    targets=[f'ytsearchdate{per_query}:{q}' for q in queries]
    targets += [_youtube_target(ch) for ch in (topic_cfg.get('channels') or [])]
    targets += topic_cfg.get('extra_sources') or []
    found=[]; seen=set()
    with _ydl({'extract_flat':'in_playlist','skip_download':True,'playlistend':per_query}) as ydl:
        for target in (t for t in targets if t):
            try: info=ydl.extract_info(target,download=False)
            except Exception as exc: logger.warning('SOURCE_LIST_FAIL target=%s error=%s',target,str(exc)[:180]); continue
...
def download(candidate:Dict,dest_dir:Optional[Path]=None)->Optional[Path]:
    """Download a bounded vetting copy, not an oversized original.

    Vetting only needs enough pixels for motion/OCR. The renderer consumes this
    same bounded file, so max_download_height and max_download_bytes are the
    hard guardrails that keep a run from bloating disk and RAM.
    """
    dest_dir=ensure_dir(dest_dir or config.clips_dir); stem=safe_slug(f"{candidate.get('source_id') or ''}_{candidate.get('title') or 'clip'}"); template=str(dest_dir/f'{stem}.%(ext)s')
    height=int(config.get('max_download_height',720) or 720); max_bytes=int(config.get('max_download_bytes',250*1024*1024) or 0)
