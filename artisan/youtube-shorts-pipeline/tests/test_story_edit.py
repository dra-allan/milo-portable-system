...
from src.video_editor import build_background_filters  # noqa: E402
...
# ---------------------------------------------------------------------------
# The regex / signal tests
# ---------------------------------------------------------------------------
def test_payoff_lexicon_actually_matches_words():
    """r'\\\\b(...)' matched a literal backslash, so this never fired."""
    assert proc.PAYOFF_RE.search('wait for it')
    assert proc.PAYOFF_RE.search('that was insane')
    assert proc.PAYOFF_RE.search('top spot')
    assert not proc.PAYOFF_RE.search('completely unrelated sentence')


def test_word_tokeniser_actually_matches_words():
    """The empty token list short-circuited _score_text to all zeros."""
    assert proc.WORD_RE.findall("don't stop believing") == ["don't", 'stop',
                                                            'believing']


def test_score_text_is_no_longer_uniformly_zero():
    setup, payoff, relevance, density = proc.score_text(
...
def test_score_text_still_returns_zeros_for_empty_speech():
    assert proc.score_text('', ['x']) == (0.0, 0.0, 0.0, 0.0)
...
def test_build_background_filters_with_in_label():
    filters, out_label = build_background_filters('crop', in_label='evcat')
    assert any('[evcat]' in f for f in filters)
    assert not any('[0:v]' in f for f in filters)
    assert out_label == 'padded'

    # Hardcoded default [0:v] remains unchanged for straight/legacy
    def_filters, def_out_label = build_background_filters('crop')
    assert any('[0:v]' in f for f in def_filters)
    assert def_out_label == 'padded'
