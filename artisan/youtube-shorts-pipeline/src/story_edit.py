...
def _register_caption_preset() -> None:
    ...
    Registered here rather than edited into ``captions`` so the caption
...
def build_plan(window: Dict, segments: Sequence[Dict], min_duration: float,
    ...
    requested = (style or getattr(config, 'edit_style', STYLE_AUTO) or STYLE_AUTO).lower()
    if requested not in STYLES:
        requested = STYLE_AUTO

    notes: List[str] = []
    min_span = float(getattr(config, 'edit_min_span', 0.6))
    hook_tail = float(getattr(config, 'hook_tail_seconds', 1.2))
    teaser = float(getattr(config, 'cold_open_seconds', 1.8))
    ...
    title_hook = ''
    if getattr(config, 'hook_text_enabled', True) and hook_text:
        title_hook = title_hook_from(
            hook_text,
            uppercase=getattr(config, 'hook_uppercase', True),
            max_words=getattr(config, 'hook_max_words', 9),
        )
