...
The logo goes on *after* the text so a wide text line can never sit on top of the
campaign's branding. Several campaigns require the logo to be visible; none of
them require the text to be able to cover it.
...
can list ``h264_nvenc`` and ``h264_qsv`` while neither actually runs (no GPU or no
driver), and trusting the list fails *every* clip in a run with "Unknown
...
edges, and the platform re-encode preserves those blocks rather than hiding them.
Speed here comes from parallel renders and hardware offload, not from a degraded
picture.
"""
...
def _encode_args() -> List[str]:
    ...
    vocabulary, so each gets its own mapping. ``VIDEO_CRF`` therefore keeps
    meaning roughly the same thing regardless of which encoder was picked.
    """
...
def render_clip(spec: CampaignSpec, plan: Dict, copy: Dict,
    ...
    The report carries the artefacts the validator needs (sheet paths and ink
    counts, whether the logo stage ran) so validation can check what actually
    happened instead of re-deriving intent from the spec.
    """
    ...
    chains: List[str] = list(ov.fill_chain('0:v', 'filled'))
    label = 'filled'
    if sheets:
    ...
    args = [
        ...
        # on a long source that is minutes per clip.
        '-ss', f"{float(plan['start']):.3f}",
        '-t', f'{duration:.3f}',
        '-i', str(source),
    ]
    if not has_audio:
        # A real generated track, not just an audio codec flag. Several
        # platforms treat a video with no audio stream at all as a broken
        # upload, and these source folders do contain silent clips.
        args += ['-f', 'lavfi', '-t', f'{duration:.3f}', '-i', SILENT_INPUT]

    args += ['-filter_complex', ';'.join(chains), '-map', '[vout]']
    if has_audio:
        args += ['-map', '0:a:0?', '-c:a', 'aac',
                 '-b:a', config.audio_bitrate, '-ar', '48000', '-ac', '2']
    else:
    ...
    args += [
        ...
        # near a campaign minimum is the difference between compliant and
        # rejected.
        '-t', f'{duration:.3f}',
        str(out_path),
    ]
...
def cleanup_work(report: Dict) -> None:
    ...
    Only the temp dir. The output MP4 has to survive until its link is actually
    accepted by the campaign board, which is a separate step that can fail.
    """
