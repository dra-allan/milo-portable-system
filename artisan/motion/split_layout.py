...
Vendored from openshorts split_layout.py (MIT) on 2026-08-25, per the
self-contained rule. Adaptations from upstream:
  ...
  * face detection goes through sibling ``face_detect`` (mediapipe-or-YuNet)
    instead of openshorts' main.py globals.
...
def analyze_scene(frames, frame_w):
    ...
    ``face_detect.detect_face_candidates`` returns). Returns ``(left, right)``
    face centres as (cx, cy) tuples, or None.
    """
    if not frames:
        return None
...
def detect_split_scenes(video_path, scenes, strategies, samples=None):
    ...
    EXCLUSIVE ends. Returns ``{scene_index: (left_centre, right_centre)}``.
    Only scenes the existing classifier sent to GENERAL are considered: a
    GENERAL verdict already means "more than one face", and leaving TRACK alone
    keeps this from touching the single-speaker material that is most of the
    corpus.
    """
