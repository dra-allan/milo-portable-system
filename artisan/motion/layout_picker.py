...
Vendored from openshorts layout_picker.py (MIT) on 2026-08-25, per the
...
# does it say about OUR videos" for $0.002 and ~2s per video, with no way to
...
def pick(video_path, video_duration):
    ...
    print("   [motion] choosing a layout for this video")
    try:
        # Inside the try on purpose: the contract above is that this never
        # raises, and an unimportable SDK/pydantic is just one more reason to
        # fall back.
        ...
        if not frames:
            print("   [motion] no readable frames — keeping default layout")
            return "none"
        ...
        response = client.models.generate_content(
            model=model_name,
            contents=parts + [gemini_layout.LAYOUT_CHOICE_PROMPT],
            config=genai_types.GenerateContentConfig(
                response_mime_type="application/json",
                response_schema=gemini_layout.LayoutChoice,
            ))
        gemini_layout.raise_if_blocked(response)
        answer = json.loads(response.text) or {}
    except Exception as e:
        print(f"   [motion] layout choice failed ({e}) — keeping "
              "default layout")
        return "none"

    decision = str(answer.get("layout", "none")).strip().lower()
    if decision not in VALID:
        print(f"   [motion] unknown layout '{decision}' — keeping default")
        return "none"

    why = str(answer.get("why", ""))[:80]
    confidence = answer.get("confidence")
    print(f"   [motion] layout: {decision} (confidence {confidence}) — {why}")
    return decision
...
def pick_and_apply(video_path, video_duration):
    ...
    if touched:
        print(f"   [motion] enabled: {', '.join(touched)}")
    return decision
