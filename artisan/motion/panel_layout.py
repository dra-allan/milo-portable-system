...
Vendored from openshorts panel_layout.py (MIT) on 2026-08-25, per the
self-contained rule. Adaptations from upstream:

  * scenes arrive as (start_frame, end_frame) int pairs — this package's
    scene_detection contract — not PySceneDetect FrameTimecode pairs.
  * face detection goes through sibling ``face_detect`` instead of openshorts'
    main.py globals.
...
Geometry, for a 1080x1920 output:
    ...
    3 people   three cells of a 2x2 grid, 540x960 each (fourth = the room)
    4 people   a 2x2 grid, 540x960 each

Three stacked bands was the obvious shape and it does not work — see the note
on ``tile_grid``. Both grids keep every tile the same size: a bigger tile
silently claims the person in it is the important one, and the layout has no
way to know that.
...
def analyze_scene(frames, frame_w):
    ...
    ``face_detect.detect_face_candidates`` returns them.
    """
    if not frames:
        return None
...
def detect_panel_scenes(video_path, scenes, strategies, samples=None):
    ...
    EXCLUSIVE ends. Only scenes the classifier already sent to GENERAL are
    considered, for the same reason SPLIT does it: GENERAL already means "more
    than one face", and single-speaker material is most of the corpus and must
    not be touched.
    """
    # Gate above the imports on purpose: cv2 is heavy and the disabled path
    # (the default, and what CI exercises) must not pay it.
    if not ENABLED:
        return {}
