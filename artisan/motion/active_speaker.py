...
says somebody is speaking at all â€” otherwise a nod, a laugh or a chewed sweet
scores as speech.
...
# "unclear" and simply does not vote â€” which is the honest answer when both are
...
def _as_box(centre):
    """[x, y, w, h] box from a (cx, cy, w, h) centre tuple.

    Local copy of split_layout.as_box: the render-layout module was not worth
    vendoring for one function.
    """
    cx, cy, w, h = centre
...
def verdicts_for_scene(video_path, start_f, end_f, fps, centres):
    ...
    boxes = [_as_box(c) for c in centres]
