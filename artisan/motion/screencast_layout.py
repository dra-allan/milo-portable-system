...
Vendored from openshorts screencast_layout.py (MIT) on 2026-08-25, per the
self-contained rule. Adaptations from upstream:

  * scenes arrive as (start_frame, end_frame) int pairs — this package's
    scene_detection contract.
  * full-resolution face detection lives in sibling ``face_detect``
    (``detect_face_candidates_full_res``); the YOLO person fallback is sibling
    ``person_detect.detect_person``.
  * prompts/schemas come from sibling ``gemini_layout`` (vendored subset of
    upstream gemini_worker).
  * plain ``[motion]`` log prefixes instead of emoji.
...
edge density and MSER text density BOTH score ordinary detailed footage higher
than a clean panel of text, because they measure visual busyness rather than
meaning. The third attempt asked Gemini and detected well, but decided badly:
it flagged corner tickers and demoted well-framed talking heads to the blurred
layout.

What is different here is the question asked and what the answer is used for.

  - The question is how much of the WIDTH the content spans, not how much of
    the duration it covers. Coverage did not separate the cases (screencasts
    ran 88-97% of the video, a corner-ticker clip 2%, but the failure was on
    the ticker anyway). Width is the quantity that actually decides whether a
    9:16 crop destroys information: a corner bug spans ~15% of the frame and
    survives any crop, a spreadsheet spans ~100% and cannot.
...
def detect_content_ranges(video_path, video_duration):
    ...
    import gemini_layout

    model_name = os.environ.get("GEMINI_MODEL") or 'gemini-3.1-flash-lite'
    print("   [motion] checking for full-width on-screen content")
    try:
        # Inside the try on purpose: an unimportable SDK/pydantic is just one
        # more reason to fall back to face-only routing.
        ...
        while True:
            ...
            if state == "FAILED" or time.time() > deadline:
                print("   [motion] upload not usable — keeping face-only routing")
                return []
            time.sleep(2)
        ...
        response = client.models.generate_content(
            ...
            contents=[file_upload,
                      gemini_layout.WIDE_CONTENT_PROMPT_TEMPLATE.format(
                          video_duration=video_duration)],
            config=genai_types.GenerateContentConfig(
                response_mime_type="application/json",
                response_schema=gemini_layout.WideContentResponse,
            ))
        gemini_layout.raise_if_blocked(response)
        raw = (json.loads(response.text) or {}).get("ranges") or []
    except Exception as e:
        print(f"   [motion] on-screen check failed ({e}) — keeping "
              "face-only routing")
        return []
    ...
    if ranges:
        shown = ", ".join(f"{w}@{s:.0f}-{e:.0f}s ({frac:.0%} wide)"
                          for s, e, w, frac in ranges[:5])
        print(f"   [motion] wide content: {shown}")
    else:
        print("   [motion] no full-width content — routing unchanged")
    return ranges
...
def detect_screencast_scenes(video_path, scenes, strategies, ranges, samples=6):
    ...
    ``scenes`` is this package's [(start_frame, end_frame), ...] contract with
    EXCLUSIVE ends. Returns ``{scene_index: ('SCREENCAST', centre) | ('WIDE',
    None)}``:
    ...
    if not ENABLED or not ranges:
        return {}
    # Below the gate on purpose: cv2 and the detectors are heavy, and the
    # disabled path (the default, and what CI exercises) must not pay them.
    import cv2
    ...
    try:
        for i, (s_f, e_f) in enumerate(scenes):
            ...
            for f_idx in np.linspace(s_f, last_f, samples):
                ...
                if not ok:
                    continue
                centre = _face_centre(
                    detect_face_candidates_full_res(frame), frame_w)
                if centre is None:
                    # A presenter keyed into the corner of a screen recording is
                    # often too small even at full resolution (measured: zero
                    # detections across an Excel walkthrough where the person
                    # is plainly visible). YOLO finds the body in the same
                    # frames, and a body centre frames the speaker just as
                    # well for this layout.
